...
class NewsEncoder(nn.Module):
    def __init__(self, cfg, glove_emb=None):
        ...
        if cfg.dataset.dataset_lang == 'english':
            ...
            # Load the parquet file using pandas and convert it to a numpy array of type float32
            pretrain_df = pd.read_parquet('data/bert_base_multilingual_cased.parquet')
            
            # Ensure all data in the DataFrame is numeric
            pretrain_df = pretrain_df.apply(pd.to_numeric, errors='coerce')
            pretrain_df = pretrain_df.fillna(0)  # Replace NaNs with zeros or an appropriate value
            
            pretrain_np = pretrain_df.to_numpy().astype(np.float32)
            pretrain = torch.from_numpy(pretrain_np)
            self.word_encoder = nn.Embedding.from_pretrained(pretrain, freeze=False, padding_idx=0)
        else:
